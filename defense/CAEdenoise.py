...
def CAEdenoise(adv_example):
    ...
    with sess1.as_default():
        with CAE_graph.as_default():
            ...
            ckpt_state = tf.train.get_checkpoint_state('./model/CAEdenoise_model/')
            # 检测模型是否已训练
            if ckpt_state:
            ...
            # 若模型没有被训练，则训练模型
                ...
                for i in range(epochs):
                    for ii in range(mnist.train.num_examples // batch_size):
                        ...
                        if ii % 100 == 0:
                            print("训练次数 {0}/{1}...训练损失 {2}".format(i, epochs, batch_cost))

                # 保存模型
                saver = tf.train.Saver()
                saver.save(sess1, './model/CAEdenoise_model/model')
            # 绘制训练误差图
            '''
            plt.plot(err)
            plt.xlabel("训练次数")
            ...
            noisy_imgs = adv_example[:10]
            reconstructed = sess1.run(decoded, feed_dict={X_noisy: noisy_imgs.reshape((10, 28, 28, 1))})
