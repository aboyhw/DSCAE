...
# 计算散度
def k1_div(rho, rho_hat):
    term2_num = tf.constant(1.) - rho
    term2_den = tf.constant(1.) - rho_hat
    k1 = logfunc(rho, rho_hat) + logfunc(term2_num, term2_den)
    return k1
...
def spcae(adv_example):
    # 定义计算图并新开一个session,避免和原有的session冲突
    space_graph = tf.Graph()
    sess1 = tf.Session(graph=space_graph)
    with sess1.as_default():
        with space_graph.as_default():
            # g1 = tf.Graph()
            # 网络参数
            # 图像的高度和宽度
            h_in, w_in = 28, 28
            ...
            logits1 = tf.layers.conv2d(encoded, 3, (k, k), padding='same', activation=None)
            encoded_sig = tf.nn.sigmoid(logits1, name='encoded_sig')
            # Decoder
            ...
            k1_div_loss = tf.reduce_sum(k1_div(0.02, tf.reduce_mean(encoded_sig, 0)))
            loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=X, logits=logits)
            cost = tf.reduce_mean(loss) + alpha * k1_div_loss
